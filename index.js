...
// ── Cache index.html template for OG injection ──────────────────────────────
let htmlTemplate = '';
try {
  htmlTemplate = fs.readFileSync(path.join(DIST, 'index.html'), 'utf8');
  logger.info('Template HTML chargé en cache');
} catch (err) {
  logger.warn('Impossible de charger index.html en cache');
}
...
app.get('/market/:id', async (req, res) => {
  ...
  if (htmlTemplate) {
    return res.send(htmlTemplate.replace('<head>', `<head>${ogTags}`));
  }
...
if (require.main === module) {
  ensureMongo().then(() => {
    ...
    const { startSyncer } = require('./src/agents/polymarketSyncer');
    startResolver();
    startTrending();
    startPipeline();
    startSnapshotJob();
