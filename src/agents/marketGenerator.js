...
async function isDuplicate(title) {
  ...
  // 1. Check in-memory cache for recent duplicates in the same cycle
  ...
  // 2. Check MongoDB for persistent deduplication
  try {
    const Market = require('../../db/models/Market');
    // Simple case-insensitive match on title. In production, a text index would be better.
    const existing = await Market.findOne({ title: { $regex: new RegExp(`^${title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') } }).select('_id').lean();
    return !!existing;
  } catch (err) {
    logger.warn(`marketGenerator: dedup check error — ${err.message}`);
    return false;
  }
}
...
async function filterMarkets(markets, trend) {
  ...
  for (const m of markets) {
    ...
    const duplicate = await isDuplicate(m.title);
    if (duplicate) {
...
function _mockGenerate(trend) {
  ...
  // Mock can't easily do async dedup here without changing signature, 
  // but it's okay for mock fallback.
  return [{
    title,
...
async function generateFromTrends(trends) {
  ...
  logger.info(`marketGenerator (NATHAN): processing ${batch.length} trends in parallel`);
